...
a2=sigmoid(W1*data+bias_param1);
a3=sigmoid(W2*a2+bias_param2);

%平方差项
ave_square=sum(sum(a3-data).^2)./(2*data_size(2));
%正则项
weight_decay=lambda*(sum(sum(W1.^2))+sum(sum(W2.^2)))./2;
%平均激活度，sum（，2）是按行相加，分子是所有样本对特定一个隐藏神经元的平均激活度的向量
rou=zeros(hiddenSize,1);
rou=sum(a2,2)./data_size(2);
p_para=repmat(sparsityParam,hiddenSize,1);
sparsity=beta.*sum( p_para.*log(p_para./rou)+(1-p_para).*log((1-p_para)./(1-rou)) );
...
delta2=((W2')*delta3+spDt*ones(1,data_size(2))).*a2.*(1-a2);
